      ...
      {orders.map((order) => (
        <Card key={order.orderid} className="w-full">
          <CardHeader>
            <div className="flex items-baseline justify-between">
              <div className="flex items-baseline space-x-4">
                ...
                <div className="flex-col items-center">
                  ...
                  <Badge variant="outline" className="w-18">{order.paymentMethod}</Badge>
                </div>
              </div>
              <div className="flex items-baseline">
                <div className="ml-4 mt-4">
                  <div className="text-xl font-semibold">₹{order.totalPrice.toFixed(2)}</div>
                  {!order.status && (
                    <Button
                      variant="default"
                      size="sm"
            ...
            {order.paymentURL && (
              <Dialog>
                <DialogTrigger className="w-full flex justify-start text-blue-600">Receipt</DialogTrigger>
                <DialogContent className="mt-6 flex justify-center">
                  <img src={order.paymentURL} alt="Receipt" className="h-96 w-38" />
                </DialogContent>
              </Dialog>
            )}
          </CardHeader>
...
export default function OrderSummary() {
  ...
  const updateOrderStatus = async (orderId: number, newStatus: boolean) => {
    try {
      ...
      await axios.put(BASE_URL + `/api/orderSummary/update/${orderId}?status=${newStatus}`)
  ...
  <div className="container mx-auto p-6 pt-0">
    <Tabs defaultValue="pending" className="w-full">
      ...
      <TabsContent value="pending">
        <OrderList orders={pendingOrders} updatingStatus={updatingStatus} updateOrderStatus={updateOrderStatus} />
      </TabsContent>

      <TabsContent value="completed">
        <OrderList orders={completedOrders} updatingStatus={updatingStatus} updateOrderStatus={updateOrderStatus} />
      </TabsContent>
    </Tabs>
  </div>
</ScrollArea>
