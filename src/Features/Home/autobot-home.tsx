...
const RpaUploadPage = () => {
  const [files, setFiles] = useState<{
    ...
    oracle: File | null;
    pennant: File | null;
  }>({ finacle: null, oracle: null, pennant: null });

  const [message, setMessage] = useState('');

  const handleFileChange = (key: 'finacle' | 'oracle' | 'pennant', e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleUpload = async () => {
    ...
    if (files.finacle) formData.append("finacleFile", files.finacle);
    if (files.oracle) formData.append("oracleGlFile", files.oracle);
    if (files.pennant) formData.append("pennetFile", files.pennant);
    ...
    <div className="p-8 max-w-2xl mx-auto bg-white shadow-md rounded">
      ...
      <div className="space-y-6">
        ...
        <div className="border p-4 bg-gray-50">
          <label htmlFor="finacle-upload" className="block font-bold mb-2">1. Upload Finacle Data</label>
          {/* Notice this input is NOT hidden. It is standard and visible. */}
          <input 
            type="file" 
            id="finacle-upload" 
            onChange={(e) => handleFileChange('finacle', e)}
